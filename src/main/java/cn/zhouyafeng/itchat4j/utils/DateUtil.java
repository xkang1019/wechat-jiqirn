...
public class DateUtil {
    ...
    public static Integer getTodayShortToLone() {
        Date currentTime = new Date();
        SimpleDateFormat formatter = new SimpleDateFormat("yyyyMMdd");
        String dateString = formatter.format(currentTime);
    ...
    public static Integer getTodayShort(Date nowDate) {
        Date currentTime = nowDate;
        SimpleDateFormat formatter = new SimpleDateFormat("yyyyMMdd");
        String dateString = formatter.format(currentTime);
        return Integer.valueOf(dateString);
    }
...
//下面的就是把当前日期加一个月
    ...
    public static Integer getWeekStr(Date date) {
        Integer result =0;
         String str = DateUtil.getWeek(date);
        if ("星期一".equals(str)) {
            result = 1;
...
//    sdate = VeStr.Replace(sdate, "/", "-");
     ...
     * @return
     */
    public static Date getDateToWeek(String week){

        //是否是今天，且小于于10点
        if (week.equals(getWeekStr(new Date()))&& Integer.parseInt(getHour())<10){
            return new Date();
        }

        if(getDayBetweenTwoDate(new Date(),strToDate(getWeek(getStringDateShort(),week)))){
           return  strToDate(getWeek(getNextDay(getStringDateShort(),"7"),week));
        }else {
         return  strToDate(getWeek(getStringDateShort(),week));
        }
    }

    public static void main(String[] args) {
        System.out.println(getEndDateOfMonth("2019-02-24").substring(8,10));
